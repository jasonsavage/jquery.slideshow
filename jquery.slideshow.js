...
(function ($)
{
	...
	var settings = {
		...
		/**
         * Sets the number of slides to show at a time.
         * @param itemsPerPage [int] - default is 1
         */
		slidesPerPage : 1,
    ...
    var methods = {
        ...
        length : function()
        {
            return Math.ceil($slides.length/settings.slidesPerPage);
        },
	...
	function addNav( type )
	{
		...
		if(s.menu !== null)
		{
			...
			{
			    (function(index) //keep i in the correct scope
			    {
    				var $b = $( settings.html.menuBtn.replace(/\{i\}/g, i+1) )
    							.appendTo($m)
    							.on('click', function(e) {
    								e.preventDefault();
    								methods.index(index);
